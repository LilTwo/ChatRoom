...
void my_send(SOCKET client,const wstring &message){
    ...
    send(client,message.c_str(),message.length()*sizeof(message[0]),0);
}
...
void SocketServer::create_private_client(struct sockaddr_in server_addr){
    ...
    if (connect(private_server,(const struct sockaddr *)&server_addr,sizeof(server_addr)) < 0) {
        perror("connect");
        exit(-1);
    }
    char buff[1024];
    while(true){
        ...
        send(private_server,L"psuedo client",max_len,0);
    }
}

SocketServer::SocketServer(int port,const string& host){
    max_len = 1024;
    ...
    while(true){
        ...
        for(auto name_socket:name2socket){
            ...
            if(FD_ISSET(client,&recv_set)){
                wchar_t buff[1024];
                memset(buff,0,1024);
                auto r = recv(client,buff,1024,0);
...
void SocketServer::register_new_client(wstring &name,SOCKET client){
    ...
    public_message(name + L" have entered the room");
...
void SocketServer::on_new_client(SOCKET client){
    ...
    while(true){
        ...
        auto r = recv(client,buff,max_len,0);
        if( r == 0){
...
void SocketServer::on_client_leave(SOCKET client){
    ...
    public_message(name + wstring(L" have leaved the room"));
}
